...
Screenshots come in phone/desktop PAIRS (a `<div class="pair">` of two <figure>s)
where the two layouts differ, and single images where they don't. Sized so the
...
body = re.sub(r'src="(manual/img/[^"]+)"', inline_img, body)

logo_b64 = base64.b64encode((HERE.parent / 'app/assets/brand/sabeel-logo.png').read_bytes()).decode()
...
html = f"""<!doctype html><html><head><meta charset="utf-8"><style>
  ...
  .cover-logo {{ width: 96mm; border: none; }}
  ...
  /* ---- Single image (layouts identical across devices) ---- */
  img {{ display: block; max-width: 82mm; margin: 12px auto;
         border: 1px solid #DFD1C1; border-radius: 8px; }}

  /* ---- Phone + desktop pair, side by side ---- */
  .pair {{ display: flex; gap: 7mm; align-items: flex-start; justify-content: center;
  ...
  .pair figcaption {{ font-size: 9.5pt; color: #6A5748; font-style: italic;
                      margin-top: 4px; }}
  .pair img {{ margin: 0; max-width: none; }}
  .pair img.wide {{ width: 116mm; }}
  .pair img.narrow {{ width: 46mm; }}
